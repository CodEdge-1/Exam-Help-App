...
# ---- Check pip ----
echo ""
echo -e "${BOLD}[2/6] Upgrading pip...${RESET}"
...
if [ "$PLATFORM" = "Linux" ]; then
    echo ""
    echo -e "${BOLD}[3/6] Installing Linux dependencies...${RESET}"
    ...
    echo ""
    echo -e "${BOLD}[3/6] Checking macOS dependencies...${RESET}"
    echo -e "${GREEN}✅ macOS dependencies OK${RESET}"
fi

# ---- Install Python packages ----
echo ""
echo -e "${BOLD}[4/6] Installing Python packages (1-2 minutes)...${RESET}"
$PYTHON -m pip install -r requirements.txt
echo -e "${GREEN}✅ All packages installed${RESET}"

# ---- Open Firewall for port 8000 ----
echo ""
echo -e "${BOLD}[5/6] Opening firewall on port 8000...${RESET}"
if [ "$PLATFORM" = "Mac" ]; then
    PYTHON_BIN=$(command -v python3 || command -v python)
    if sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add "$PYTHON_BIN" >/dev/null 2>&1 && \
       sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp "$PYTHON_BIN" >/dev/null 2>&1; then
        echo -e "${GREEN}✅ macOS firewall - Python allowed (port 8000 open)${RESET}"
    else
        echo -e "${YELLOW}⚠️  Could not update firewall. Run with sudo to fix mobile access.${RESET}"
    fi
elif [ "$PLATFORM" = "Linux" ]; then
    if command -v ufw &>/dev/null; then
        sudo ufw allow 8000/tcp >/dev/null 2>&1 && \
            echo -e "${GREEN}✅ ufw rule added - port 8000 open${RESET}" || \
            echo -e "${YELLOW}⚠️  Could not add ufw rule. Try: sudo ufw allow 8000/tcp${RESET}"
    elif command -v firewall-cmd &>/dev/null; then
        sudo firewall-cmd --add-port=8000/tcp --permanent >/dev/null 2>&1 && \
        sudo firewall-cmd --reload >/dev/null 2>&1 && \
            echo -e "${GREEN}✅ firewalld rule added - port 8000 open${RESET}" || \
            echo -e "${YELLOW}⚠️  Could not add firewalld rule. Try: sudo firewall-cmd --add-port=8000/tcp --permanent${RESET}"
    else
        echo -e "${YELLOW}⚠️  No supported firewall found. Manually open port 8000 if needed.${RESET}"
    fi
fi

# ---- Create directories ----
echo ""
echo -e "${BOLD}[6/6] Creating folders...${RESET}"
