...
# ---- Ensure firewall port 8000 is open for mobile access ----
if [[ "$(uname)" == "Darwin" ]]; then
    PYTHON_BIN=$(command -v python3 || command -v python)
    sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add "$PYTHON_BIN" >/dev/null 2>&1
    sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp "$PYTHON_BIN" >/dev/null 2>&1
else
